...
patterns = [
    re.compile(r'ya29\.[A-Za-z0-9_\-\.]{10,}'),   # google access token style
    re.compile(r'AIza[0-9A-Za-z\-_]{10,}'),       # google api key style
    re.compile(r'1//[A-Za-z0-9_\-\.]{10,}'),      # refresh token-ish
    re.compile(r'[A-Za-z0-9_\-]{40,}'),           # long base64/hex-like strings
]
        ...
        for row in rows:
            ...
            for idx, val in enumerate(row[1:], start=1):
                if isinstance(val, (str, bytes)):
                    ...
                    m = match_any(s)
                    if m:
                        found.append((t, rowid, cols[idx-1], m, s[:300]))
    except:
        pass
